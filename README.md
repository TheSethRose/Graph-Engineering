...
Bare `agent-workflow` starts a run in the current directory and prompts for the task in the TUI; `agent-workflow run` remains an explicit alias. `--task "..."` is available for scripts or when the task is already written, and `--repo` can target another path. The TUI fills the available terminal height with live commands, file changes, validation results, and final responses; it suppresses command output and source-code contents. Press `t` to toggle that activity view and `p` to pause after the active graph node, then continue, add corrective guidance, or abort. Use `--no-interactive` for structured output, which requires `--task`; `--verbose` adds command progress and heartbeats, while `--trace` includes the filtered activity in non-interactive output. Hermes one-shot mode exposes only its final response, not internal tool calls. `status` reads checkpoints without executing the graph, while `resume` requires a response allowed by the active interrupt and rejects a changed repository fingerprint.
...
A new run requires a named branch but does not require an initial commit and may start from a dirty worktree. For a repository with commits, it creates an isolated linked worktree; for an unborn repository, it creates a standalone Git workspace. Both forms seed the current tracked and non-ignored untracked files under the user data root, record a private Git index as the run baseline, and execute every worker and validation command there. On successful completion it checks the workflow-only patch against the source repository before applying it; a conflict fails safely and preserves the isolated workspace for inspection.
