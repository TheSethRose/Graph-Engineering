...
## Why TypeScript and Node.js fit
...
Node.js also provides the required CLI and subprocess primitives without a framework. Target Node.js 24 LTS for a stable compatibility baseline. Version one remains explicitly macOS/Unix because validation uses `/bin/sh` and the process-lock recovery contract must be tested per operating system.
...
The official JavaScript persistence documentation describes `@langchain/langgraph-checkpoint-sqlite` as a SQLite checkpointer suited to experimentation and local workflows. That is a fit, not a defect, while this tool runs one local workflow at a time.
...
Validation commands are a separate, explicitly trusted code boundary. Version one accepts them only from repeated caller `--validate` flags or a Git-tracked `.agent-workflow.json` at the repository root; planner suggestions are never executable input. CLI values replace the JSON list. Run these commands sequentially through `/bin/sh -lc`; package scripts already execute arbitrary repository code, so a partial shell parser would add complexity without creating a meaningful security boundary.
...
- Keep validation deterministic, with only CLI or checked-in JSON as trusted sources.
